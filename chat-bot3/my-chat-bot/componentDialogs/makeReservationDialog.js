...
class MakeReservationDialog extends ComponentDialog {
    ...
    async getPowerOptions(step) {
        ...
        var reply = MessageFactory.suggestedActions(['Available', 'Not Available'], 'select power status');
        //var reply = "yes ,No"
    ...
    async getEquipments(step) {
        ...
        if (step.values.inPower == 'Available') {
            ...
            var equipment = MessageFactory.suggestedActions(['POE', 'Modem', 'MUX'], 'select equipment');
    ...
    async inPowerOptions(step) {
        ...
        if (myapp.flag == true) {
            if (step.values.equip == "POE") {
                ...
                var poe = MessageFactory.suggestedActions(['Red', 'Green', 'Amber'], 'POE light indication');
                ...
                myapp.flag2 = true;
                //console.log("result"+step.values.inPower)
                var modem = MessageFactory.suggestedActions(['Red', 'Green', 'Amber'], 'Modem light indication');
                ...
                myapp.flag2 = true;
                //console.log("result"+step.values.inPower)
                var mux = MessageFactory.suggestedActions(['Red', 'Green', 'Amber'], 'MUX light indication');
    ...
    async routerPortLight(step) {
        ...
        if (myapp.flag3 == 1) {
            myapp.flag4 = 1;
            var rpl = MessageFactory.suggestedActions(['Admin light', 'Data light', 'Both'], 'Select rout port light indication');
    ...
    async visible(step) {
        ...
        if (myapp.flag4 == 1) {
            ...
            if (step.values.rpl == 'Admin light' || step.values.rpl == 'Data light' || step.values.rpl == 'Both') {
                var joji = MessageFactory.suggestedActions(['yes', 'no'], 'JOJI device at POE');
    ...
    async check(step) {
        ...
        if (myapp.flag5 == 1) {
            if (step.values.joji == 'yes') {
                ...
                myapp.flag6 = 'b';
                var chk = MessageFactory.suggestedActions(['yes', 'no'], 'POE rebooted');
                return await step.prompt(TEXT_PROMPT, chk)
            }
        }
        else if (myapp.flag5 == 0) {
            myapp.flag6 = 'a';
            var rt = MessageFactory.suggestedActions(['yes', 'no'], 'roof top permission availablity');
            endDialog = true;
